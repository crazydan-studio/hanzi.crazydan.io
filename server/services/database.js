...
export function initDatabase(dbPath = DB_PATH) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS characters (
      ...
      pinyin TEXT NOT NULL DEFAULT '[]',        -- 读音 JSON 数组（含声调，可多音）
...
function migrateStrokeCharId() {
  ...
  if (charCols.some(c => c.name === 'meaning')) return   // 旧表未迁移，跳过
...
export function serializeCharacter(row) {
  if (!row) return null
  const { created_at, updated_at, pinyin, pinyin_plain, ...rest } = row
  let pinyinArr = [], plainArr = []
  try { pinyinArr = JSON.parse(pinyin) } catch { pinyinArr = pinyin ? [pinyin] : [] }
  try { plainArr = JSON.parse(pinyin_plain) } catch { plainArr = pinyin_plain ? [pinyin_plain] : [] }
  return {
    ...rest,
    pinyin: pinyinArr,
    pinyin_plain: plainArr,
...
export function serializeStroke(row) {
  if (!row) return null
  const { trajectory_data, created_at, updated_at, ...rest } = row
  return {
    ...rest,
    trajectory_data: JSON.parse(trajectory_data),
    created_at: toISO(created_at),
    updated_at: toISO(updated_at)
  }
}
