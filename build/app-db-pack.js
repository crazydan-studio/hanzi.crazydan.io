...
//         hanzi.db.sha256
// 说明:
//   - 汉字信息（meta_zi 实体表，zi 为视图: 汉字经 char(id) 计算，不在实体表冗余存储）
//     在打包时单独导出，笔画数据（strokes 表）不打包
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        `)
        // zi 为视图: 汉字由 id（unicode 数值）经 char(id) 计算，不冗余存储
        out.exec(`
          CREATE VIEW zi AS
        ...
        const ins = out.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)`)
        const rows = db.prepare('SELECT id, pinyin, used_weight, structure, radical, total_stroke_count, is_traditional FROM zi').all()
        for (const r of rows) {
          ins.run(r.id, r.pinyin, r.used_weight ?? 0,
