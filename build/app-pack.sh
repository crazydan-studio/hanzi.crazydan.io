...
#   3. 打包开发数据库 server/data/hanzi_stroke.db → app/android/src/main/assets/db/hanzi.db
#   4. 构建 Android App（Gradle，Compose Multiplatform 原生 UI）
#   5. 安装包移至 public/assets/app/android/（保留最新构建的安装包）
...
echo "==> [3/5] 打包数据库到 app 资源目录"
(cd "${ROOT}" && node build/app-db-pack.js)

echo "==> [4/5] 构建 Android App（${BUILD_TYPE}，版本 ${VERSION_NAME}）"
(cd "${APP_DIR}" && ./gradlew ":android:${TASK}")

echo "==> [5/5] 移动安装包到 public/assets/app/android/"
